...
void PostEffectHighLumi::Draw(ID3D12GraphicsCommandList* cmdList)
{
	...
	cmdList->SetGraphicsRootDescriptorTable(1, PostRenderBase::GetInstance()->GetSrvDesc()->GetGPUDescriptorHandleForHeapStart());
...
void PostEffectHighLumi::PreDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	...
	D3D12_CPU_DESCRIPTOR_HANDLE rtvH =
		PostRenderBase::GetInstance()->GetRtvDesc()->GetCPUDescriptorHandleForHeapStart();

	// 深度ステンシルビュー用デスクリプタヒープのハンドルを取得
	D3D12_CPU_DESCRIPTOR_HANDLE dsvH =
		PostRenderBase::GetInstance()->GetDsvDesc()->GetCPUDescriptorHandleForHeapStart();
...
{
	PostRenderBase::GetInstance()->CreateSRV(texBuff_.Get(), handles_.srvCpuHandle_);
}
