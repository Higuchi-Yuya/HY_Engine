...
Vector2 Random::RangeVec2(const float min, const float max, const bool isSameValue)
{
	if (isSameValue == true)
	{
...
Vector3 Random::RangeVec3(const float min, const float max, const bool isSameValue)
{
	if (isSameValue == true)
	{
		const float randomValue = RangeF(min, max);
...
Vector2 Random::RangeInCircle(const float minRadius, const float maxRadius)
{
