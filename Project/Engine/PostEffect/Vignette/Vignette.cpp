...
ID3D12Device* Vignette::sDevice_ = nullptr;

const float Vignette::clearColor_[4] = { 0.4f,1.0f,0.4f,0.0f };// ミドリっぽい色
...
void Vignette::Initialize()
{
	...
	PostEffectHandleManager::SetPostEffectHandle("Vignette", handles_);
...
{
	constMap_->vignetteColor = vignetteColor_;
	constMap_->vignetteRadius= vignetteRadius_;
	constMap_->vignetteFeather = vignetteFeather_;
}

void Vignette::ImguiUpdate()
{
	// 表示項目の追加--------//
	...
	ImGui::ColorEdit4("vignetteColor", &vignetteColor_.x);

	ImGui::SliderFloat("vignetteRadius", &vignetteRadius_, 0, 1);

	ImGui::SliderFloat("vignetteFeather", &vignetteFeather_, 0, 1);
...
void Vignette::Draw(ID3D12GraphicsCommandList* cmdList)
{
	...
	cmdList->SetGraphicsRootDescriptorTable(0, handles_.srvGpuHandle_);
...
void Vignette::SetDevice(ID3D12Device* device)
{
	sDevice_ = device;
}

void Vignette::CreateVertBuff()
{
	// 頂点バッファの設定
	// ヒープ
	...
	// リソースデスク
	CD3DX12_RESOURCE_DESC constResDesc = CD3DX12_RESOURCE_DESC::Buffer((sizeof(ConstBufferDataVignette) + 0xff) & ~0xff);
...
void Vignette::CreateGraphicsPipelineState()
{
	...
	vsShader_->Create("PostEffect/Vignette/VignetteVS.hlsl", "main", "vs_5_0", ShaderObj::ShaderType::VS);

	// ピクセルシェーダの読み込みとコンパイル
	psShader_ = std::make_unique<ShaderObj>();
	psShader_->Create("PostEffect/Vignette/VignettePS.hlsl", "main", "ps_5_0", ShaderObj::ShaderType::PS);
