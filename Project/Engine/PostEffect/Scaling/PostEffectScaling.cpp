...
ID3D12Device* PostEffectScaling::sDevice_ = nullptr;

const float PostEffectScaling::clearColor_[4] = { 0.4f,1.0f,0.4f,0.0f };// ミドリっぽい色

PostEffectScaling::PostEffectScaling()
{
}

void PostEffectScaling::Initialize()
{
	CreateVertBuff();
	...
	PostEffectHandleManager::SetPostEffectHandle("scaling", handles_);
}

void PostEffectScaling::Update()
{
	constMap_->tailing = tailing_;
	constMap_->offset = offset_;
	constMap_->IsActive = IsActive_;
	constMap_->alpha = alpha_;
}

void PostEffectScaling::ImguiUpdate()
{

}

void PostEffectScaling::Draw(ID3D12GraphicsCommandList* cmdList)
{
	// パイプラインステートの設定
...
void PostEffectScaling::PreDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	// リソースバリアを作成
...
void PostEffectScaling::PostDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	// リソースバリアを作成
...
void PostEffectScaling::SetDevice(ID3D12Device* device)
{
	sDevice_ = device;
}

void PostEffectScaling::CreateVertBuff()
{
	// 頂点バッファの設定
	// ヒープ
	...
	// リソースデスク
	CD3DX12_RESOURCE_DESC constResDesc = CD3DX12_RESOURCE_DESC::Buffer((sizeof(ConstBufferDataScaling) + 0xff) & ~0xff);
...
void PostEffectScaling::CreateTex()
{
	// テクスチャリソース設定
...
void PostEffectScaling::CreateDepthBuff()
{
	...
	depthBuff_->SetName(L"PostScaling");
}

void PostEffectScaling::CreateDSV()
{
	PostRenderBase::GetInstance()->CreateDSV(depthBuff_.Get(), handles_.dsvCpuHandle_, DXGI_FORMAT_D32_FLOAT_S8X24_UINT);
}

void PostEffectScaling::CreateGraphicsPipelineState()
{
	result = S_FALSE;
	...
	vsShader_->Create("PostEffect/Scaling/ScalingVS.hlsl", "main", "vs_5_0", ShaderObj::ShaderType::VS);

	// ピクセルシェーダの読み込みとコンパイル
	psShader_ = std::make_unique<ShaderObj>();
	psShader_->Create("PostEffect/Scaling/ScalingPS.hlsl", "main", "ps_5_0", ShaderObj::ShaderType::PS);
